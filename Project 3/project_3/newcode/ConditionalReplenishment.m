...
for i = 1:length(delta)  % loop over step sizes of quantizer
    ...
    for j = 1:length(V)-1 % loop over all frames, comparison between two frames: -1
        ...
        Vblk16_1 = mat2cell(V{j}, repmat(16, 1, frame_size(2)/16), repmat(16, 1, frame_size(1)/16)); %divide each frame into blocks of 16x16
        Vblk16_2 = mat2cell(V{j+1}, repmat(16, 1, frame_size(2)/16), repmat(16, 1, frame_size(1)/16)); %divide each frame into blocks of 16x16
        [M,N] = size(Vblk16_1);
        
        for ii = 1:M
            for jj =1:N
                [R_intra,~,  Dist_intra] = encode_intraframe(Vblk16_1{ii,jj}, delta(i));
                R_intra = R_intra + 1;
